...
export default function EventConsole3D({ onSend, onEventCreated }: EventConsole3DProps) {
  ...
  const checkEventProof = async (eventId: string) => {
    try {
      ...
      const response = await fetch(`/api/proofs/${eventId}`)
      let proofData: ProofResponse

      if (response.ok) {
        proofData = await response.json()
      } else {
        // Mock successful proof for demo
        proofData = {
          event_id: eventId,
          proof: ["hash1", "hash2", "hash3"],
          root: "root_hash_" + eventId,
          length: 3,
        }
      }

      // Update orb status to verified
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-sm font-medium">
                          {JSON.parse(event.data.actor).user_id || "unknown"}
                        </span>
